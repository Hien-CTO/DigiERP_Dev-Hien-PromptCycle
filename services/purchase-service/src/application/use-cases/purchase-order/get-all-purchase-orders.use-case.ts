...
export class GetAllPurchaseOrdersUseCase {
  ...
  async execute(page: number = 1, limit: number = 10, search?: string): Promise<PurchaseOrderResponseDto[]> {
    const orders = await this.purchaseOrderRepository.findAll(page, limit, search);
  ...
  private toResponseDto(order: PurchaseOrder): PurchaseOrderResponseDto {
    return {
      ...
      purchase_request_id: order.purchaseRequestId, // Map purchase_request_id
