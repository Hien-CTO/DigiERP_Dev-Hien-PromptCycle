...
export class ApprovePurchaseRequestUseCase {
  ...
  async execute(id: string, approvedBy: string): Promise<PurchaseRequestResponseDto> {
    ...
    const approvedRequest = request.approve(approvedBy);
    const savedRequest = await this.purchaseRequestRepository.update(approvedRequest);
